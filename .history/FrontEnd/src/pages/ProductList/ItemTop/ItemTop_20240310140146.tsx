    ...
    >
      {listItem?.length > 0 &&
        listItem?.slice(10, 18)?.map((product, index) => (
          ...
          >
            <Link to={`${path.home}${generateNameId({ name: data?.data.data.products.name, id: product._id })}`}>
              {' '}
              <SwiperItemTop products={product} key={data?.data.data.products._id}></SwiperItemTop>
            </Link>
          </SwiperSlide>
        ))}
    </Swiper>

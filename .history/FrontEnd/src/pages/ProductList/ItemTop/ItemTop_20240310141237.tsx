    ...
    >
      {listItem?.length > 0 &&
        listItem?.slice(10, 18)?.map((product, index) => (
          console.log(data?.data.data.products[index].name);
          <SwiperSlide
            key={index}
            className=' flex flex-col items-center border justify-center px-2 shadow-none hover:shadow-lg w-full relative '
          >
            <Link
              to={`${path.home}${generateNameId({ name: data?.data.data.products[index].name, id: data?.data.data.products[index]._id })}`}
            >
              {' '}

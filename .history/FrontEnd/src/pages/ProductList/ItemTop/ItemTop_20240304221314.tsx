...
function ItemTop({ data }: Props): FC<AppProps> {
  ...
  return (
    ...
    >
      {listItem?.length > 0 &&  listItem?.slice(0, 4)?.map((image,view,name
, index) =>(
        <SwiperSlide key={index} className=' flex flex-col items-center border justify-center px-2 shadow-none hover:shadow-lg w-full relative '>
          <SwiperItemTop

...
function FormAccount() {
  ...
  const [isEditModalVisible, setIsEditModalVisible] = useState(false) // State để kiểm soát hiển thị form chỉnh sửa
  const [selectedUserId, setSelectedUserId] = useState<string>('') // State để lưu trữ id của người dùng được chọn
  ...
  const handleEdit = (userId: string) => {
    setSelectedUserId(userId) // Lưu trữ id của người dùng được chọn
  ...
  if (usersData) {
    ...
    return (
      <div>
        ...
        >
          <CollectionEditForm userId={selectedUserId} onClose={() => setIsEditModalVisible(false)} />
        </Modal>
        <Table
          pagination={{

...
const CollectionCreateForm: React.FC<CollectionCreateFormProps> = ({ onFormInstanceReady, onImageDataReceived }) => {
  ...
  const handleIllustrationImageUpload = (imageUrl: string[]) => {
    // Kiểm tra và chuyển đổi imageUrl thành một mảng các chuỗi nếu cần
    const imagesArray: string[] = Array.isArray(imageUrl) ? imageUrl : [imageUrl]

    // Gửi mảng imagesArray đã kiểm tra và chuyển đổi đến setFieldsValue
    form.setFieldsValue({ images: imagesArray })
  }

...
const CollectionCreateForm: React.FC<CollectionCreateFormProps> = ({ onFormInstanceReady, onImageDataReceived }) => {
  ...
  const [illustrationImageData, setIllustrationImageData] = useState<any>(null)
  const handleMainImageUpload = (data: any) => {
    setMainImageData(data)
    onImageDataReceived({ mainImageData: data }) // Truyền dữ liệu lên component cha
  }
  console.log(handleMainImageUpload)

  const handleIllustrationImageUpload = (data: any) => {
    setIllustrationImageData(data)
    onImageDataReceived({ illustrationImageData: data }) // Truyền dữ liệu lên component cha
  }
  return (
    <Form
      {...formItemLayout}

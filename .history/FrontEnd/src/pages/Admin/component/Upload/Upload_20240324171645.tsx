...
const Uploadimgs: React.FC<UploadimgsProps> = ({ onUpload }) => {
  ...
  const onChange: UploadProps['onChange'] = async ({ fileList: newFileList }) => {
    ...
    const filteredUrls: string[] = uploadedDataArray.filter((url) => url.endsWith('.jpg')).map((url) => url.toString()) // Chuyển đổi URL thành chuỗi

    setFileList(updatedFileList)
    onUpload(filteredUrls)
    console.log(typeof filteredUrls)
  }

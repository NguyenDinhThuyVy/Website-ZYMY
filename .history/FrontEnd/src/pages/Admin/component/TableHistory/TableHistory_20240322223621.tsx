  ...
  const columns: TableProps<Order>['columns'] = [
    ...
    },
    {
      ...
      render: (status: number, record: any) => {
        const handleStatusChange = async (newStatus: number) => {
          ...
          try {
            if (newStatus === 3) {
              ...
              await adminApi.confirmaccept([record.key])
              console.log(`Order ${record.key} đã được xác nhận.`)
            } else if (newStatus === 5) {
              // Gọi API confirmcancel
              await adminApi.confirmcancel([record.key])
        ...
        return (
          <Select defaultValue={Đang chuẩn bị} onChange={handleStatusChange}>

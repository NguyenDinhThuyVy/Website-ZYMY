...
import { useQuery, UseQueryResult } from 'react-query' // Import UseQueryResult
import useQueryConfig from 'src/hooks/useQueryConfig'
import { Order } from 'src/types/order.type'
import { useEffect, useState } from 'react'

// Tạo một hàm tùy chỉnh để gọi useQuery
function useUserQuery(userId: string): UseQueryResult<any> {
  return useQuery({
  ...
  useEffect(() => {
    if (orderData && orderData.data && orderData.data.data) {
      ...
      const queries = dataIndex.map((userId) => {
        return useUserQuery(userId) // Gọi useUserQuery thay vì useQuery
      })

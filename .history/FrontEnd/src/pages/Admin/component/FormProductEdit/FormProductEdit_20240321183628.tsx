...
const FormProductEdit: React.FC<CollectionCreateFormProps> = ({ productId, onClose, onUpdateSuccess }) => {
  ...
  const handleUploadChange1 = async (info: any) => {
    ...
    console.log('hihi')
    form.setFieldsValue({
      images: uploadRes.data.data // Sử dụng đường dẫn của ảnh từ dữ liệu phản hồi
    })
    setProductData((prevProductData) => ({
      ...prevProductData,
      data: {
        ...prevProductData.data,
        images: getAvatarUrl(uploadRes.data.data)
      }
    }))
  }
  ...
  const initialFileList = images ? convertUrlsToFileList(productData?.data.images) : []
          ...
          <Form.Item label='Ảnh minh họa' name='images'>
            <Upload
              ...
              onRemove={(file) => {
                ...
                if (indexToRemove !== -1) {
                  // Tạo một bản sao của fileList hiện tại
                  const updatedFileList = [...initialFileList]
                  // Xóa phần tử tương ứng khỏi fileList
                  updatedFileList.splice(indexToRemove, 1)
                  // Cập nhật fileList bằng fileList đã được cập nhật
                  setInitialFileList(updatedFileList)

                  // Xóa ảnh khỏi state productData
                  setProductData((prevProductData) => ({
                    ...prevProductData,
                    data: {
                      ...prevProductData.data,
            ...
            >
              {(productData?.data.images ?? []).map((image, index) =>
                ...
                image ? null : <div key={`upload-${index}`}>Upload</div>
              )}
            </Upload>
          </Form.Item>
        </div>

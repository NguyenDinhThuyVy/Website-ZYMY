...
const FormProductEdit: React.FC<CollectionCreateFormProps> = ({ productId, onClose, onUpdateSuccess }) => {
  ...
  const convertUrlsToFileList = (urls: string[]): { uid: string; name: string; url: string }[] => {
    // Lọc ra các URL hợp lệ và chỉ trích tên tệp từ mỗi URL
    const validUrls = urls.filter((url) => url.startsWith('http://localhost:4000/images/'))
    const fileList = validUrls.map((url, index) => {
      const filename = url.substring(url.lastIndexOf('/') + 1) // Lấy tên tệp từ URL
      return {
        uid: `-${index}`,
        name: filename,
        url: url
  ...
  return (
    ...
    >
      <Form {...formItemLayout} form={form} initialValues={productData}>
        ...
        <Form.Item label='Mô tả sản phẩm' name='description' className='w-[800px] py-[20px] '>
          <Input.TextArea style={{ width: '1300px', height: '200px', rows: 10 }} />
        </Form.Item>
      </Form>
    </Modal>
  )

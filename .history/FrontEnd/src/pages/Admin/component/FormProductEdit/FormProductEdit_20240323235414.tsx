...
const FormProductEdit: React.FC<CollectionCreateFormProps> = ({ productId, onClose, onUpdateSuccess }) => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        setProductData(productDataResponse.data.data)
  ...
  const uploadImageMutaion = useMutation(adminApi.updateImage)
  ...
  const customRequest = async ({ file, onSuccess, onError }: any) => {
    ...
    try {
      const response = await fetch('http://localhost:4000/admin/products/update-image', {
        method: 'PUT',
        headers: {
      ...
      <Form {...formItemLayout} form={form} initialValues={productData}>
        ...
        <div className='grid grid-cols-2 grid-flow-row  w-full'>
          <Form.Item label='Ảnh chính' name='image' initialValue={image}>
            <Upload
              action='http://localhost:4000/admin/products/update-image'

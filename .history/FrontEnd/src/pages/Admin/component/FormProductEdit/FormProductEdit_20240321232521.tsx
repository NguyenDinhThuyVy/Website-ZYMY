...
const FormProductEdit: React.FC<CollectionCreateFormProps> = ({ productId, onClose, onUpdateSuccess }) => {
  ...
  const handleUploadChange = async (info: any) => {
    ...
    form.setFieldsValue({
      images: productData?.data.images.map((image, index) => {
        ...
        if (index === indexToRemove && uploadRes && uploadRes.data && uploadRes.data.data) {
  ...
  const handleUploadChange1 = async (info: any) => {
    ...
    console.log('hihi')
    form.setFieldsValue({
      images: ImagesArray(productData?.data.images, indexToRemove, uploadRes) // Sử dụng đường dẫn của ảnh từ dữ liệu phản hồi
    })
    setProductData((prevProductData) => ({
      ...prevProductData,
      data: {

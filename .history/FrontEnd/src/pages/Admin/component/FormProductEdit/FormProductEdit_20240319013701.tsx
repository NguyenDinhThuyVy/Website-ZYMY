...
import { Form, GetProp, Image, Input, Modal, Select, Upload, UploadFile, UploadProps } from 'antd'
...
import Image from 'antd/lib/image/image'
...
const FormProductEdit: React.FC<CollectionCreateFormProps> = ({ productId, onClose, onUpdateSuccess }) => {
  ...
  const onPreview = async (file: UploadFile) => {
    let src: string | null = file.url

    // Kiểm tra URL ảnh
    if (!src) {
      // Đọc file thành DataURL nếu không có URL
      try {
        const reader = new FileReader()
        const blob = file.originFileObj as Blob // Kiểm tra kiểu dữ liệu
        const promise: Promise<string> = new Promise((resolve) => {
          reader.readAsDataURL(blob)
          reader.onload = () => resolve(reader.result as string)
          reader.onerror = (error) => {
            console.error('Error reading file:', error)
            // Xử lý lỗi đọc file (tùy chọn)
          }
        })
        src = await promise
      } catch (error) {
        console.error('Error creating preview:', error)
        // Xử lý lỗi tổng thể (tùy chọn)
      }
    }

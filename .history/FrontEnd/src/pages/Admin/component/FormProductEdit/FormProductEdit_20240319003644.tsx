...
const FormProductEdit: React.FC<CollectionCreateFormProps> = ({ productId, onClose, onUpdateSuccess }) => {
  ...
  const [imageUrl, setImageUrl] = useState<string>('') // State để lưu trữ đường dẫn hình ảnh
  ...
  return (
    ...
    >
      <Form {...formItemLayout} form={form} initialValues={productData}>
        ...
        <div className='grid grid-cols-2 grid-flow-row  w-full'>
          <Form.Item label='Ảnh chính' name={'image'} initialValue={productData.data.data.image}>
            {/* Không cần sử dụng Uploadmain */}
          </Form.Item>

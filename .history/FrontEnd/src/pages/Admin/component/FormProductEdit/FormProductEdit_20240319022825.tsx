      ...
      <Form {...formItemLayout} form={form} initialValues={productData}>
        ...
        <div className='grid grid-cols-2 grid-flow-row  w-full'>
          <Form.Item label='Ảnh chính' name='image' initialValue={image}>
            <Upload
              ...
              onChange={(info) => {
                ...
                if (status === 'done' && response && response.url) {
                  ...
                  // Xóa ảnh hiện tại trong productData khi đã upload ảnh mới thành công
            ...
            >
              {productData && productData.data.image ? (
                <img src={productData.data.image} alt='Product' style={{ maxWidth: '100%', maxHeight: '100%' }} />
              ) : (
                <div>Upload</div>
              )}
            </Upload>

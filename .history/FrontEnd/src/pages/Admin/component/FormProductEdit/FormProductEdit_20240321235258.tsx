...
const FormProductEdit: React.FC<CollectionCreateFormProps> = ({ productId, onClose, onUpdateSuccess }) => {
  ...
  const convertUrlsToFileList = (urls: string[]): { uid: string; name: string; url: string }[] => {
    const normalizedUrls = urls.map((url) => url.trim()) // Chuẩn hóa các URL bằng cách loại bỏ các khoảng trắng ở đầu và cuối
    const uniqueUrls = Array.from(new Set(normalizedUrls)) // Lọc ra các URL duy nhất

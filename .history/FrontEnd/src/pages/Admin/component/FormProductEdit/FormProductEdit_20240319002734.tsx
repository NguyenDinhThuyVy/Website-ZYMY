...
const FormProductEdit: React.FC<CollectionCreateFormProps> = ({ productId, onClose, onUpdateSuccess, imageUrl }) => {
  const [form] = Form.useForm()

  const [productData, setProductData] = useState<any>(null)
  const [imageLink, setImageLink] = useState<string>('') // State để lưu trữ đường dẫn hình ảnh
  useEffect(() => {
      ...
      <Form {...formItemLayout} form={form} initialValues={productData}>
        ...
        <div className='grid grid-cols-2 grid-flow-row  w-full'>
          <Form.Item label='Ảnh chính' name={'image'}>
            <Uploadmain imageUrl={imageLink} onChange={(link: string) => setImageLink(link)} />
          </Form.Item>

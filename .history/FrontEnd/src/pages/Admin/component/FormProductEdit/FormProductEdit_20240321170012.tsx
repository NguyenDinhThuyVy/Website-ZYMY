...
const FormProductEdit: React.FC<CollectionCreateFormProps> = ({ productId, onClose, onUpdateSuccess }) => {
  ...
  const handleUploadChange = async (info) => {
    ...
    if (status === 'done') {
      ...
      try {
        ...
        const imageUrl = uploadRes.data.data.url // Lấy URL của hình ảnh đã tải lên từ phản hồi API
        form.setFieldsValue({ image: imageUrl }) // Thiết lập giá trị của trường image trong form
        // setProductData((prevProductData) => ({
        //   ...prevProductData,
        //   data: {
        //     ...prevProductData.data,
        //     image: imageUrl // Cập nhật giá trị image trong state productData
        //   }
        // }));
      } catch (error) {
  ...
  const customRequest = async ({ file, onSuccess, onError }) => {
    ...
    try {
      ...
      const responseData = await response.json() // Parse phản hồi thành JSON
      onSuccess(responseData, file) // Gọi hàm onSuccess khi upload thành công và chuyển dữ liệu phản hồi và file
    } catch (error) {

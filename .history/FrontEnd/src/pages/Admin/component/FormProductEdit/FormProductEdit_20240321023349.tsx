          ...
          <Form.Item label='Ảnh chính' name='image' initialValue={image}>
            <Upload
              ...
              onChange={async (info) => {
                ...
                if (status === 'done' && response.data && response.url) {
                  const formData = new FormData()
                  formData.append('image', info.file.originFileObj || '') // Truyền vào blob của file hoặc một giá trị rỗng nếu không có file
                  try {
                    const uploadRes = await adminApi.uploadImage(formData)
                    form.setFieldsValue({ image: uploadRes.data.data }) // Đảm bảo rằng 'image' là tên trường của bạn
                  } catch (error) {

...
const FormProductEdit: React.FC<CollectionCreateFormProps> = ({ productId, onClose, onUpdateSuccess }) => {
  ...
  const [initialCategoryValue, setInitialCategoryValue] = useState(['category', 'name'])
    ...
    >
      <Form {...formItemLayout} form={form} initialValues={productData}>
        ...
        <div className='grid grid-cols-2 grid-flow-row  w-full'>
          ...
          <Form.Item label='Danh mục' name={['category']} initialValue={['category', 'name']}>
        ...
        <div className='grid grid-cols-2 grid-flow-row  w-full'>
          <Form.Item label='Ảnh chính' name='image' initialValue={initialCategoryValue}>
            <Upload

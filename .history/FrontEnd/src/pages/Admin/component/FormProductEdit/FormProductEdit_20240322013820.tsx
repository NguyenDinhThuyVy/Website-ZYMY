...
const FormProductEdit: React.FC<CollectionCreateFormProps> = ({ productId, onClose, onUpdateSuccess }) => {
  ...
  const [hasFetchedFileList, setHasFetchedFileList] = useState(false)
  useEffect(() => {
  ...
  const convertUrlsToUploadFileList = (urls: string[]): UploadFile[] => {
  ...
  useEffect(() => {
    if (!hasFetchedFileList && productData && productData.data && Array.isArray(productData.data.images)) {
      const imagesAsUploadFile = convertUrlsToUploadFileList(productData.data.images)
      setFileList(imagesAsUploadFile)
      setHasFetchedFileList(true)
    } else {
      // Xử lý trường hợp productData hoặc productData.data hoặc productData.data.images không tồn tại
      console.error('productData or productData.data or productData.data.images is null or undefined.')
    }
  }, [productData, hasFetchedFileList])
  return (
    <Modal
      open
      width={1200}
      ...
      <Form {...formItemLayout} form={form} initialValues={productData}>
        ...
        <div className='grid grid-cols-2 grid-flow-row  w-full'>
          ...
          <Form.Item label='Ảnh minh họa' name='images'>
            <Upload action='http://localhost:4000/admin/products/upload-images' listType='picture-card'></Upload>
          </Form.Item>
        </div>

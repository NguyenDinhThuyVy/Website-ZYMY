...
const FormProductEdit: React.FC<CollectionCreateFormProps> = ({ productId, onClose, onUpdateSuccess }) => {
  ...
  const initialCategory = categoriesData?.data.data.find((category) => category.name === initialCategoryValue[0])?.name
      ...
      <Form {...formItemLayout} form={form} initialValues={productData}>
        ...
        <div className='grid grid-cols-2 grid-flow-row  w-full'>
          ...
          <Form.Item
            label='Danh mục'
            name={['category']}
            initialValue={categoriesData?.data.data.find((category) => category.name === initialCategoryValue[0])?.name}
          >

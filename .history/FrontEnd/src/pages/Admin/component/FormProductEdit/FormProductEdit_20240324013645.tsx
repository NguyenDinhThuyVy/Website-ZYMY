...
const FormProductEdit: React.FC<CollectionCreateFormProps> = ({ productId, onClose, onUpdateSuccess }) => {
  ...
  const initialCategory = categoriesData?.data.data.map((category) => {
    if (category.name === initialCategoryValue[0]) {
    ...
    >
      <Form {...formItemLayout} form={form} initialValues={productData}>
        ...
        <div className='grid grid-cols-2 grid-flow-row  w-full'>
          ...
          <Form.Item label='Danh mục' name={['category']} initialValue={initialCategory}>
            <Select style={{ width: 340 }} initialValue={initialCategory}>
              {categoriesData?.data.data.map((category) => (
                <Select.Option key={category._id} value={category._id} initialValue={initialCategory}>
                  {category.name}
                </Select.Option>
              ))}
            </Select>

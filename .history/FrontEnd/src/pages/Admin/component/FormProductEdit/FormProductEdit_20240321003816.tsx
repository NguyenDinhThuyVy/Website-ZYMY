...
const FormProductEdit: React.FC<CollectionCreateFormProps> = ({ productId, onClose, onUpdateSuccess }) => {
  ...
  const [imageURL, setImageURL] = useState<string | null>(null) // Trạng thái để lưu URL của ảnh
  useEffect(() => {
  ...
  const handleRemoveImage = () => {
    setImageURL(null) // Đặt URL của ảnh thành null khi người dùng xóa
  }
        ...
        <div className='grid grid-cols-2 grid-flow-row w-full'>
          <Form.Item label='Ảnh chính' name='image' initialValue={productData?.data.image}>
            <Upload
              ...
              maxCount={1}
              fileList={productData?.data.image ? [{ uid: '-1', name: 'image.png', url: productData?.data.image }] : []}
              onRemove={handleRemoveImage} // Sử dụng hàm xử lý khi xóa ảnh
              onChange={(info) => {
                const { status, response } = info.file
                if (status === 'done' && response && response.url) {
                  setImageURL(response.url) // Cập nhật URL của ảnh khi người dùng tải lên thành công
                }
              }}
            >
              {productData?.data.image ? null : <div>Upload</div>}
            </Upload>
          </Form.Item>

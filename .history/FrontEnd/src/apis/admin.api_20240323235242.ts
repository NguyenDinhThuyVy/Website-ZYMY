...
const adminApi = {
  ...
  updateImage(body: any) {
    ...
    // Kiểm tra xem có dữ liệu hình ảnh được cung cấp không
    if (body.image) {
      formData.append('image', body.image)
    }

    // Bây giờ gửi yêu cầu PUT với dữ liệu FormData (có hoặc không có hình ảnh)
    return http.put<SuccessResponse<string>>('/admin/products/update-image', formData, {
      headers: {
  ...
  uploadImages(body: any) {
    return http.put<SuccessResponse<string>>('/admin/products/update-image', body, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    })

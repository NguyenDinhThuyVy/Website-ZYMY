...
function ProtectedRoute({ isAdmin = false, ...rest }: ProtectedRouteProps) {
  const { isAuthenticated, isAdmin: contextIsAdmin } = useContext(AppContext)
  const hasAdminAccess = isAdmin ? isAdmin : contextIsAdmin // Check both isAdmin prop and context value

  return isAuthenticated && hasAdminAccess ? <Outlet /> : <Navigate to='/login' />
}

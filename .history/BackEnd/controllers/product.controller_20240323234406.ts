...
import { updateFile, uploadFile, uploadManyFile } from '../utils/upload'
...
const updateProductImage = async (req: Request, res: Response) => {
  try {
    const path = await updateFile(req, FOLDERS.PRODUCT)
    const response = {
    ...
    return res.status(500).json({ message: 'Có lỗi xảy ra khi cập nhật ảnh' })
  }
}
